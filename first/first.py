...
if __name__ == "__main__":
    ...
    for i,j in enumerate(path_B0):
        ...
        ax1.hist(df['piminus_PT'], bins=50, color='blue', label='$\pi^{-}_{PT}$')
        ax1.hist(df['Kplus_PT'], bins=50, color='red', label='$K^{+}_{PT}$')
        ax1.legend(loc='best')
        ax1.set_xlabel('$Energy~(MeV)$')
        ax1.set_ylabel('$Pseudorapidity$')
        ax1.minorticks_on()
        ax2=plt.subplot(1,2,2)
        ax2.hist(df['muplus_PT'], bins=50, color='blue', label='$\mu^{+}_{PT}$')
        ax2.hist(df['muminus_PT'], bins=50, color='red', label='$\mu^{-}_{PT}$')
        ax2.legend(loc='best')
...
for i,j in enumerate(path_Bplus):
        ...
        ax1=plt.subplot(1,2,1)
        ax1.hist(df['Kplus_PT'], bins=50, color='blue', label='$K^{+}_{PT}$')
        ax1.legend(loc='best')
        ax1.set_xlabel('$Energy~(MeV)$')
        ax1.set_ylabel('$Pseudorapidity$')
        ax1.minorticks_on()
